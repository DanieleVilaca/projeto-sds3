...
public interface SaleRepository extends JpaRepository<Sale, Long> {
	...
	List<SaleSumDTO> amountGroupedBySeller(); // Importando a soma agrupada por vendedor

	@Query("SELECT new com.devsuperior.dsvendas.dto.SaleSuccessDTO(obj.seller,SUM(obj.visited),SUM(obj.deals)) "
			+ " FROM Sale AS obj  GROUP BY obj.seller")
	List<SaleSuccessDTO> sucessGroupedBySeller(); // Importando a soma de visitas e de vendas 
